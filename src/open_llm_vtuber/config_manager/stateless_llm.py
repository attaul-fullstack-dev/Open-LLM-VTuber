...
class MistralConfig(OpenAICompatibleConfig):
    ...
    top_p: float | None = Field(None, ge=0, le=1, alias="top_p")
    max_tokens: int | None = Field(None, gt=0, alias="max_tokens")
    interrupt_method: Literal["system", "user"] = Field(
        "user", alias="interrupt_method"
    )

    _MISTRAL_DESCRIPTIONS: ClassVar[dict[str, Description]] = {
        "top_p": Description(
            en="Optional nucleus sampling probability for Mistral (between 0 and 1).",
            zh="Mistral 可选的核采样概率（0 到 1 之间）。",
        ),
        "max_tokens": Description(
            en="Optional maximum number of output tokens generated by Mistral.",
            zh="Mistral 可选的最大输出 token 数。",
        ),
    }

    DESCRIPTIONS: ClassVar[dict[str, Description]] = {
        **OpenAICompatibleConfig.DESCRIPTIONS,
        **_MISTRAL_DESCRIPTIONS,
    }
...
class GroqConfig(OpenAICompatibleConfig):
    ...
    _GROQ_DESCRIPTIONS: ClassVar[dict[str, Description]] = {
        "top_p": Description(
            en="Optional nucleus sampling probability for Groq (between 0 and 1).",
            zh="Groq 可选的核采样概率（0 到 1 之间）。",
        ),
        "max_tokens": Description(
            en="Optional maximum number of output tokens generated by Groq.",
            zh="Groq 可选的最大输出 token 数。",
        ),
    }
