...
class OllamaConfig(OpenAICompatibleConfig):
    ...
    _OLLAMA_DESCRIPTIONS: ClassVar[dict[str, Description]] = {
        "llm_api_key": Description(
            en="API key for authentication. Ignored locally but required by Ollama Cloud.",
            zh="API 认证密钥。本地 Ollama 会忽略，Ollama Cloud 则必须提供。",
        ),
        "top_p": Description(
            en="Optional nucleus sampling probability for Ollama (between 0 and 1).",
            zh="Ollama 可选的核采样概率（0 到 1 之间）。",
        ),
        "max_tokens": Description(
            en="Optional maximum number of output tokens generated by Ollama.",
            zh="Ollama 可选的最大输出 token 数。",
        ),
